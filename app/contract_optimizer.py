...
class ContractOptimiser:
    ...
    def find_optimal_path_and_income(self) -> tuple[int, List[str]]:
        ...
        """
        income, path = self.find_optimum(0, 0)
        return income, path

    def get_next_eligible_contract_id(self, index: int = 0, start: int = 0) -> int:
    ...
    def find_optimum(self, start: int = 0, index: int = 0) -> tuple[int, List[str]]:
        ...
        """
        if start in self._solution_cache:
            income, path = self._solution_cache.get(start)
            return income, path

        next_eligible_contract_id = self.get_next_eligible_contract_id(index, start)
        if next_eligible_contract_id == len(self.contracts):
            return 0, []
        next_eligible_contract = self.sorted_contracts[next_eligible_contract_id]

        if next_eligible_contract.get("end") in self._solution_cache:
            income_1, path_1 = self._solution_cache.get(
                next_eligible_contract.get("end")
            )
        else:
            income_1, path_1 = self.find_optimum(
                next_eligible_contract.get("end"), index + 1
            )
